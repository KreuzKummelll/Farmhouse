...
struct SplitImage: View {
  ...
  func defaultImageView() -> some View {
    Image(CustomPhoto.no_stands.rawValue)
      .resizable()
...
struct SplitImage_Previews: PreviewProvider {
  static var previews: some View {
    Group {
      SplitImage(images: [])
        .frame(height: 200)
      SplitImage(images: [UIImage(named: "farmer.0")!])
        .frame(height: 100)
      SplitImage(images: [UIImage(named: "farmer.1")!])
        .frame(height: 100)
      SplitImage(images: [UIImage(named: "farmer.0")!, UIImage(named: "farmer.1")!])
        .frame(height: 100)
    }
  }
}
